...
int main(int argc, char *argv[]) {
    ...
    in_channel_layout = 2;
    int ret = swr_alloc_set_opts2(&au_convert_ctx, &pCodecCtx->ch_layout, out_sample_fmt, out_sample_rate,
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        if (packet->stream_index == audioStream) {
            ...
            while (avcodec_receive_frame(pCodecCtx, pFrame) == 0) {
                ...
                out_buffer.resize(av_samples_get_buffer_size(NULL, 2, out_nb_samples, pCodecCtx->sample_fmt, 1));
