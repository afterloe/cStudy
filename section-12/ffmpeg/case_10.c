...
// 每次读取2帧数据, 以48000个采样点一帧 2通道 16bit采样点为例
#define PCM_BUFFER_SIZE 2 * 4096 * 2 * 2
        ...
        if (av_sample_fmt_is_planar(dec_ctx->sample_fmt)) {
            for (i = 0; i < frame->nb_samples; i++)
            {
                for (ch = 0; ch < dec_ctx->ch_layout.nb_channels; ch++) {
                    ...
                    memcpy(s_audio_buf, dst_data[0] + dst_bufsize * i, data_size);
                    // fwrite(dst_data[0] + data_size * i, 1, data_size, dst);
                }
            }
        }
        else {
